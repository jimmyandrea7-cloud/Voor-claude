      ...
      <div className="max-w-5xl mx-auto px-6 py-14">
        <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
          <div>
            ...
            <h1 className="mt-3 font-serif text-4xl md:text-5xl text-[#241F1A]">The collection</h1>
            <p className="mt-2 text-[14px] text-[#6B5F4F]">Every watch you have brought in for identification.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="border border-[#E2D9C6] rounded-[4px] overflow-hidden bg-[#FFFCF5]">
            {scans.map((s, i) => {
              ...
              return (
                <motion.div key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }} className="border-b border-[#E2D9C6] last:border-b-0">
                  <Link to={analyzed ? `/report/${s.id}` : `/scan/${s.id}`} data-testid={`scan-card-${s.id}`} className="flex items-center gap-5 px-6 py-5 hover:bg-[#FBF3E2] transition-colors">
                    <div className="w-14 text-center shrink-0">
                      {conf != null && analyzed ? (
                        <>
                          <div className="font-serif text-2xl text-[#241F1A] leading-none">{conf}</div>
